...
from core.model import NeuralStyleTransferModel
import core.utils
import config
...
def total_loss(noise_features):
    ...
    return content_loss * config.CONTENT_LOSS_FACTOR + style_loss * config.STYLE_LOSS_FACTOR


def train(content_path,style_path,training_count,learning_rate):
    # 创建模型
    model = NeuralStyleTransferModel()

    # 加载内容图片
    content_image = core.utils.load_images(content_path)
    # 风格图片
    style_image = core.utils.load_images(style_path)

    # 计算出目标内容图片的内容特征备用
    global target_content_features,target_style_features,M,N
    target_content_features = model([content_image, ])['content']
    # 计算目标风格图片的风格特征
    target_style_features = model([style_image, ])['style']

    M = config.WIDTH * config.HEIGHT
    N = 3
    ...
    # 基于内容图片随机生成一张噪声图片
    noise_image = tf.Variable((content_image + np.random.uniform(-0.2, 0.2, (1, config.HEIGHT, config.WIDTH, 3))) / 2)
    ...
    # 共训练training_count个epochs
    for epoch in range(training_count):
        # 使用tqdm提示训练进度
        with tqdm(total=config.STEPS_PER_EPOCH, desc='Epoch {}/{}'.format(epoch + 1, training_count)) as pbar:
            # 每个epoch训练config.STEPS_PER_EPOCH次
            for step in range(config.STEPS_PER_EPOCH):
                ...
                pbar.update(1)
            # 每个epoch保存一次图片
            core.utils.save_image(noise_image, '{}/{}.jpg'.format(config.OUTPUT_DIR, epoch + 1))
